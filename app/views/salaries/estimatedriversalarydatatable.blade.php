	...
	@section('page_content')
		<div class="col-xs-offset-4 col-xs-8 ccordion-style1 panel-group">
			<div class="col-xs-6 ">
				...
				?>
					<a class="btn btn-sm btn-primary" href="estimatesalary?show=true">DRIVERS/HELPERS SALARY</a> &nbsp;&nbsp;
				<?php 
					}
					if(in_array(339, $jobs)){
				?>
					<a class="btn btn-sm  btn-inverse" href="estimateofficeemployeesalary?show=true"> OFFICE EMPLOYEES SALARY </a> &nbsp;&nbsp;
				<?php 
					} 
				?>
			</div>
					...
					<thead>
						...
						<?php
							...
							foreach($entities as $entity){
								...
								$recs = SalaryTransactions::where("salaryMonth","=",$values["month"])
															->where("empId","=",$entity->id)
							...
							<tr>
								<td class="center" style="font-weight: bold; vertical-align: middle">
									<label class="pos-rel">
										<input type="checkbox" class="ace" onclick="this.checked=!this.checked;" name="ids[]" id="ids_{{$i}}" value="{{$entity->id}}"/>
										<span class="lbl"></span>
									</label>
								...
								<td style="vertical-align: middle">
									<input type="text" style="max-width:70px;vertical-align: middle;" name="other_deductions[]" id="{{$i}}_other_deductions" readonly="readonly" onchange="calcSalary(this.id)" value="{{$rec->otherDeductions}}"/>	
								</td>
								<td style="vertical-align: middle;">
							...
							<tr>
								...
								<?php 
									...
									if(($entity->terminationDate!="" && $entity->terminationDate!="0000-00-00" && $date2!="01-01-1970"  && $date2>=$date3 &&  $date2<$date4)){
										...
										$net_salary = ($salary_amt/$total_days)*$actual_working_days;
										$salary_amt = round($net_salary,2);
										$net_salary = $net_salary-($leaves_amt+$due_amt);
										$net_salary =round($net_salary,2);
									}
									if(($entity->joiningDate!="" && $entity->joiningDate!="0000-00-00" && $date1!="01-01-1970"  && $date1>$date3 &&  $date1<$date4)){
										$date3_=date_create($fromdt);
								...
								<td style="vertical-align: middle">
									<input type="text" style="max-width:70px;vertical-align: middle;" name="other_deductions[]" id="{{$i}}_other_deductions" onchange="calcSalary(this.id)" value="0.00"/>	
								</td>
								<td style="vertical-align: middle;">
									<select name="pfopted[]" id="pfopted_{{$i}}" class="form-control" >
										<option value="Yes">Yes</option>
	...
	@section('inline_js')
		...
		<script type="text/javascript">
			...
			function calcSalary(id){
				...
				//alert(other_deductions);
				if(other_deductions==""){
					other_deductions=0.00;
				}
				test=parseInt(salary);
				//alert("test"+test);
