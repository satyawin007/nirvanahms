	...
	@section('inline_js')
		...
		<script type="text/javascript">
			...
			function paginate(page){
				officebranch = $('#branch').val();
				if(officebranch==""){
					alert("please select fuel station");
					return;
				}
                ...
                frdt = moment(fromdate).format('00-MM-YYYY');
                //alert(frdt);
                todt = moment(todate).format('00-MM-YYYY');
               // alert(todt);
                var start = new Date(fromdate);
               // alert("start:"+start);
                var end = new Date(todate);
               	var to= end.setMonth(end.getMonth());
                //alert("end:"+end);

                var d = new Date();
               // alert("d:"+d);
                var month = ["JAN","FEB","MAR","APR","MAY","JUNE","JULY","AUG","SEP","OCT","NOV","DEC"];
                tableHeaders += "<th>FUEL STATION</th>";
				i =0;
				tcolumns.push({"bSortable": true});
                while(start < end){
	                   if(i==0){
	                   	   var newDate = start.setMonth(start.getMonth());
	                   	   i++;
	                   }
	                   else{
	                   ...
	                   if(newDate<to){
	                	   ...
	                	   day = day+" "+start.getFullYear();
	                       tableHeaders += "<th>TOT VEHS</th><th>TOT LTRS</th>";
                ...
                tableHeaders += "<th>TOT AMT</th><th>PAID AMT</th><th>BAL</th>";
				//tcolumns.push({"bSortable": false});
