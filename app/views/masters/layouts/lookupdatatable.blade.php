	...
	@section('page_content')
		<div class="row ">
			<div class="col-xs-offset-1 col-xs-10">
				...
				<?php if(($form_info['action']=="adddepartment" && in_array(206, $jobs)) or 
	...
	@section('inline_js')
		...
		<script type="text/javascript">
			...
			function showPaymentFields(val){
				...
				$("#addfields1").html('<div style="margin-left:600px; margin-top:100px;"><i class="ace-icon fa fa-spinner fa-spin orange bigger-125" style="font-size: 250% !important;"></i></div>');
			...
			function modalEditDepartment(id, name, type, status){
				$("#name1").val(name);				
				$("#type1").val(type);
				$("#status1 option").each(function() { this.selected = (this.text == status); });
				$("#id1").val(id);		
			}
			function modalEditManufacture(id, name,short_name,description, status){
				$("#name1").val(name);	
				$("#short_name1").val(short_name);			
			...
			function modalEditDoctors(id, name, qualification,designation,depart_id,status){
				$("#name1").val(name);				
				$("#qualification1").val(qualification);
				$("#designation1").val(designation);
				$("#status1 option").each(function() { this.selected = (this.text == status); });
				$("#department1 option").each(function() { this.selected = (this.text == depart_id); });
				$("#id1").val(id);		
			}
			...
			function modalEditFuelStation(name,accountNumber,bankName,bankAccount,chequeNumber,issueDate,transactionDate, balanceAmount, depoamt, paymentType,  pmtdate, cityId, stateId, status,  id){
				...
				$("#bankaccount option").each(function() { this.selected = (this.value == bankAccount); });
			...
			function modalEditCard(id, cardNumber, cardType, cardHolderName, bank, acctId, creditLimit,points,expireDate, status){
