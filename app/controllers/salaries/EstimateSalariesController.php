...
class EstimateSalariesController extends \Controller {
	...
	{	//$values["pay"];
		$values = Input::all();
		$values['bredcum'] = "ESTIMATE DRIVERS/HELPERS SALARY";
		...
		$form_info = array();
		$form_info["name"] = "estimatesalary";
		$form_info["action"] = "estimatesalary";
		$form_info["method"] = "post";
		$form_info["class"] = "form-horizontal";
		$form_info["back_url"] = "masters";
		$form_info["bredcum"] = "ESTIMATE EMPLOYEE SALARY";
		...
		return View::make('salaries.estimatedriversalarydatatable', array("values"=>$values));
	}
	...
	{
		$values = Input::all();
		$values['bredcum'] = "ESTIMATE OFFICE EMPLOYEE SALARY";
		$values['home_url'] = 'masters';
		$values['add_url'] = '#';
		$values['form_action'] = '#';
		$values['action_val'] = '#';
		...
		$form_info = array();
		$form_info["name"] = "estimateofficeemployeesalary";
		$form_info["action"] = "estimateofficeemployeesalary";
		$form_info["method"] = "post";
		$form_info["class"] = "form-horizontal";
		$form_info["back_url"] = "masters";
		$form_info["bredcum"] = "ESTIMATE EMPLOYEE SALARY";
		...
		return View::make('salaries.estimateofficeemployeedatatable', array("values"=>$values));
	}
	...
	public function addEmployeeSalary()
	{
		if (\Request::isMethod('post'))
		{
			...
			$url = "estimatesalary?month=".$values["month"];
			if(isset($values["clientname"]) && isset($values["depot"])){
				$url = $url;
			}
			else{
				$url = "estimateofficeemployeesalary?branch=".$values["branch"]."&month=".$values["month"];
			}
			...
			foreach ($ids as $id){
				...
				$dueDeductions = 0;
				if(isset($values['due_deductions'])){
