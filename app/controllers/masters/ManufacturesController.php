...
class ManufacturesController extends \Controller {
	...
	public function addManufacturer()
	{
		...
		{
			$values = Input::all();
			$field_names = array("name"=>"name","description"=>"description","short_name"=>"short_name");
			$fields = array();
	...
	public function editManufacturer()
	{
		...
		if (\Request::isMethod('post'))
		{
			$field_names = array("name1"=>"name","description1"=>"description","short_name1"=>"short_name","status1"=>"status");
			$fields = array();
			foreach ($field_names as $key=>$val){
				if(isset($values[$key])){
					$fields[$val] = $values[$key];
	...
	public function manageManufacturers()
	{
		...
		$values['action_val'] = '#';
		$theads = array(' ID','Name',"short_name", "Description", "status", "Actions");
		...
		$form_field = array("name"=>"short_name", "content"=>"short name", "readonly"=>"","action"=>array("type"=>"onchange","script"=>"checkvalidation(this.value,this.id,'Manufacturers')"),  "required"=>"required","type"=>"text", "class"=>"form-control");
		$form_fields[] = $form_field;
		...
		$form_fields[] = $form_field;
		$form_field = array("name"=>"short_name1", "content"=>"short name", "readonly"=>"","action"=>array("type"=>"onchange","script"=>"checkvalidation(this.value,this.id,'Manufacturers')"),  "required"=>"required","type"=>"text", "class"=>"form-control");
		$form_fields[] = $form_field;
